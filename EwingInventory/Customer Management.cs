    ...
    public partial class Customer_Management : Form
    {
        ...
        private void txtaddress1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                ...
                if (address1.Equals(""))
                {
                    MessageBox.Show("Company name is not Entered ");
                ...
                else
                {
                    if (uniqueCust() == true)
                    {
                        MessageBox.Show("The company name is already exist");
                        txtaddress1.Focus();
                    }
                    else
                        txtaddress2.Focus();
            ...
            if (paytype != "" && delto != "" && seller != "")
            {
                ...
                else
                {
                    ...
                    if (confirm == DialogResult.Yes)
                    {
                        ...
                        confirm = MessageBox.Show("Do you want to Send invoice to customer", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                        if (confirm == DialogResult.Yes)
                        {
                            tabcontrol_CustMgt.SelectTab(tabmail);
                        }
                        else
                        {
        ...
        private  void clearOrder()
        {
            ...
            txtqtyDisc.Text = "";
            txtcashDisc.Text = "0";
            txttotalCost1.Text = "0";
            txttotalDisc1.Text = "0";
            txtnetCost1.Text = "0";
            txtSeller1.Text = "";
            DueDateCalc();
            cmbdeliverto.Items.Clear();
            txttotqty1.Text = "0";
            expDelDate.Value = DateTime.Now;
            txtitcode.Text = "";
            txtspecPer.Text = "";
            while (InvoiceNotable.Rows.Count > 0)
            {
        ...
        private void txtspecPer_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyValue == 13)
            {
                ...
                double cashdisc = Convert.ToDouble(txtcashDisc.Text);
                double netcost = 0, totcost = 0, specdisc = 0, totdisc = 0, totqtydisc=0;
                ...
                while (reader.Read())
                {
                    ...
                    totqtydisc = Convert.ToDouble(reader[1].ToString());
                ...
                txttotalDisc1.Text = (totqtydisc+ cashdisc + specdisc).ToString();

                netcost = totcost - (cashdisc + specdisc);
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            if (confirm == DialogResult.Yes)
            {
                ...
                if (reader.HasRows)
                {
                    ...
                    cnn.Open();
                    query = "update payment set paymentid='" + pmtid + "', date1='" + DateTime.Now.ToString() + "',EnteredBy='" + Homepage.currentUser1 + "',drawnDate='" + drawndate + "',checkNo='" + checkno + "',refNo='" + refno + "',depositeTo='" + depto + "' where invoiceNo='" + invoiceno + "';";
                    cmd = new MySqlCommand(query, cnn);
                    reader = cmd.ExecuteReader(); // query will be executed and data updated to the db
                    cnn.Close();
                ...
                else
                {
                    ...
                    for (int rows = 0; rows < pmtTable.Rows.Count - 1; rows++)
                    {
                        ...
                        cnn.Open();
                        query = "update payment set paymentid='" + pmtid + "', OutstandingAmt='" + due + "',payingAmt='" + payingamt + "',date1='" + DateTime.Now.ToString() + "',EnteredBy='" + Homepage.currentUser1 + "',drawnDate='" + drawndate + "',checkNo='" + checkno + "',refNo='" + refno + "',depositeTo='" + depto + "' where invoiceNo='" + invoiceno + "';";
                        cmd = new MySqlCommand(query, cnn);
                        reader = cmd.ExecuteReader(); // query will be executed and data updated to the db
                        cnn.Close();
        ...
        /*  foreach (DataGridViewRow Datarow in pmtIDtable.Rows)
            {
                ...
                {*/
                    ...
                    try
                    {
                        ...
                        cnn.Open();
                        MySqlDataAdapter adp = new MySqlDataAdapter("select paymentid as PaymentID,date1 as PmtDate,sum(payingAmt) as AmountPaid  from payment where cid='" + contentValuep + "' and PaymentID!='0' GROUP BY paymentid,date1;", cnn);
                        adp.Fill(dt5);
                        pmtIDtable.DataSource = dt5;
                        this.pmtIDtable.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                        this.pmtIDtable.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                ...
                query = "select sum(OutstandingAmt) from payment where cid='" + contentValuep + "' GROUP BY '" + contentValuep + "';";
                    ...
                    catch (FormatException ee)
                    {
                       ...
                       // MessageBox.Show(contentValuep);
                        ...
                        // Displaying old outstanding amount while paying in table when viewing payment details
                            for (int rows = 0; rows < pmtTable.Rows.Count - 1; rows++)
                            {
                            double payingamt = 0, outamtnew = 0;
                                for (int col = 0; col < pmtTable.Rows[rows].Cells.Count; col++)
                                {
                                    if (col == 3)
                                    {

                                         outamtnew = Convert.ToDouble(this.pmtTable.Rows[rows].Cells[col].Value.ToString());
                                    ...
                                    this.pmtTable.Rows[rows].Cells[3].Value = outamtnew + payingamt;
                                }
                            }
                    }
        ...
        private void button5_Click_2(object sender, EventArgs e)
        {
            try
            {
                ...
                att = new Attachment(txtattachment.Text);
                mail.Attachments.Add(att);
                smtpServer.Port = 587;


                smtpServer.Credentials = new System.Net.NetworkCredential(txtfrom.Text, txtpw.Text);
        ...
        private void txtcustpmtdet_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private bool uniqueCust()
        {
            string compname=txtaddress1.Text;
            query = "select address2 from customer where address2='"+compname+"' ;";
            MySqlCommand cmd = new MySqlCommand(query,cnn);
            cnn.Open();
